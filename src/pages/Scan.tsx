...
export function Scan({ engines, onSave }: Props) {
  ...
  const analyze = () => { if (!selected) { setNotice('Nejdřív vyberte motor.'); return null } const next = analyzeCar(form, engines); setResult(next); setNotice(''); return next }
  const save = () => { const current = result ?? analyze(); if (!current) return; onSave(current); setNotice('Vůz byl uložen do garáže.'); }
  ...
  return <div className="scan-layout">
    <section className="panel form-panel"><div className="step-title"><span>01</span><div><span className="eyebrow">IDENTIFIKACE</span><h2>Údaje o vozidle</h2></div></div>
      ...
      <button className="primary full-button" onClick={analyze} disabled={!selected}><i className="fa-solid fa-wand-magic-sparkles" /> Vyhodnotit vozidlo</button>
